...
def get_args():
    ...
    parser.add_argument("--model", choices=["Llama2-7B", "Llama2-13B", "Llama2-70B", "GPT3-175B", "Qwen"], help="LLM Model", required=True)
    ...
    parser.add_argument("--decoding", type=int, help="Decoding length", default=1024*4)
...
def generate_trace(args, seqlen_list):
    ...
    print(f"Generating traces for {args.model} with {args.generate_trace_max_workers} threads... {model}")
...
def load_data_point(args, seqlen, FC_devices, channels_per_block, PCIe_lanes_per_device, blocks_per_device, embedding_latency, utilized_devices, pp, tp):
    ...
    if args.model_parallel:
        if "Llama" in args.model or "Qwen" in args.model:
            ...
            print("llama_latency", cxl_latency)
        else:
    ...
    latency_denominator = stats["latency"]
    if latency_denominator <= 0:
        print(f"Warning: zero PIM latency for seqlen={seqlen}, FC_devices={FC_devices}. Power is set to 0.")
    ...
    if args.model_parallel:
        ...
        for comp in energy_main.keys():
            ...
            if latency_denominator > 0:
                power_alldv[comp] = (energy_main[comp] + energy_FC[comp] * (FC_devices - 1)) * pipeline_stages / latency_denominator
            else:
                power_alldv[comp] = 0.0
    else:
        for comp in energy_main.keys():
            energy_token[comp] = energy_main[comp] * utilized_devices
            if latency_denominator > 0:
                power_alldv[comp] = energy_main[comp] * utilized_devices / latency_denominator
            else:
                power_alldv[comp] = 0.0
    total_energy = 0
